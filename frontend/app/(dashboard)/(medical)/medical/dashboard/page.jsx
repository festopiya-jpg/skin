    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white flex items-center gap-2">
          <Pill className="text-sky-400 w-8 h-8" />
          Pharmacy / Medical Dashboard
        </h1>
        <p className="text-slate-400 mt-1">Manage doctor-approved medication orders.</p>
      </div>

      <div className="glass-card rounded-2xl overflow-hidden mt-6">
        <div className="p-6 border-b border-slate-700/50 flex justify-between items-center">
          <h2 className="text-xl font-bold text-white">Pending Prescriptions</h2>
        </div>
        <div className="overflow-x-auto p-4">
          {queue.length === 0 ? (
            <div className="text-center py-10">
              <Clock className="w-12 h-12 text-slate-500 mx-auto mb-3 opacity-50" />
              <p className="text-slate-400 font-medium">No pending medication orders.</p>
            </div>
          ) : (
              ...
              {queue.map((order) => (
                <div key={order.id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                  <div className="flex-1">
                    ...
                    <p className="text-sky-300 text-sm font-medium mb-1">Diagnosis: {order.condition}</p>
